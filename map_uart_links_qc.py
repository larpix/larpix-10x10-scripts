...
def get_good_roots(c, io_group, io_channels):
        ...
        for n, io_channel in enumerate(io_channels):
                ...
                print("setting uart clock ratio to:",clk_ctrl_2_clk_ratio_map[base._default_clk_ctrl]) 
...
def get_initial_controller(io_group, io_channels, vdda=0, pacman_version='v1rev3'):
        ...
        if pacman_version == 'v1rev2':
                ...
                print('IDDA:',idda,'mA')
                for ch in range(1,5):
                        c.io.set_reg(0x1000*ch + 0x2014, _uart_phase, io_group=1)
...
def test_chip(c, io_group, io_channel, path, ich, all_paths_copy, io_channels_copy, config):
        ...
        for direction in mover_directions:
                ...
                if next_chip in arr.excluded_chips:
                        continue
                ...
                if not ok: 
                        ...
                        base.reset(c, config, enforce=True, modify_power=False)
                        continue
...
def main(pacman_tile, io_group, skip_test, tile_id, pacman_version, vdda, exclude):
        if not exclude is None:
                exclude_chips = string_to_chips_list(exclude)
                print('Excluding chips:', exclude_chips)
                for chip in exclude_chips: arr.add_excluded_chip(chip)
        ...
        c, keys=base.main(controller_config=_name+'.json', enforce=True, return_bad_keys=True)
        c = base.reset(c, _name+'.json', enforce=True)
        if len(keys) > 0:
                for key in keys:
                        for net in paths:
                                if key in net:
                                        index = net.index(key)
                                        if index > 0: 
                                                arr.add_excluded_link( (net[index-1], net[index]) )

                return main(pacman_tile, io_group, skip_test, tile_id, pacman_version, vdda, exclude)
        ...
        ##
        config = _name+'.json'
        c=base.main(controller_config=config, enforce=True, vdda=0)
