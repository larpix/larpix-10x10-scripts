...
_default_trim_sigma_file=None
_default_disable_list=[]
_default_noise_cut=3.
_default_null_sample_time=0.25
...
def find_pedestal(pedestal_file, noise_cut, c, verbose):
    ...
    for unique in sorted(unique_channels):
        ...
        if len(adc) < 2 or np.mean(adc)>200. or np.std(adc)>noise_cut:
...
def update_chip(c, status):
    chip_register_pairs = []
    for chip_key in status.keys():
        chip_register_pairs.append( (chip_key, list(range(64))+ list(range(66,74))) )
